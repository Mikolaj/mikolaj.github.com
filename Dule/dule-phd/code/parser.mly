/* Copyright (C) 2003, 2006 Mikolaj Konarski, Aleksander Naleczynski
 *
 ...
 *
 * $Id: parser.mly,v 1.58 2006-08-07 14:45:38 mikon Exp $
 */ 
...
let mkwild () = 
  IdIndex.loc2wild (Error_rep.Location.t2string (symbol_rloc ()))
let mkpatid () = 
  IdIndex.loc2patt (Error_rep.Location.t2string (symbol_rloc ()))
...
let mk_elem_list l =
    let rec l2con acc l = 
      ...
      | [] -> acc
      | h::t -> 
	  let ht = cons (AtIndex.head, h) (cons (AtIndex.tail, acc) nil) in
	  let new_acc =
	    mkvalu(T_comp (mkvalu(T_comp (mkvalu(T_record ht), 
					  mkvalu(T_in AtIndex.cons))), 
				  mkvalu(T_con)))
	  in l2con new_acc t
    in
    l2con (mkvalu(T_comp (mkvalu(T_comp 
				   (mkvalu(T_record nil), 
				    mkvalu(T_in AtIndex.nil))),
			  mkvalu(T_con)))) l
...
%right    EQUAL
%nonassoc COLONCOLON
%nonassoc COLONGREATER MINUSGREATER
%left     WITH 
%left     TILDE
%nonassoc COLON
%nonassoc LPAREN
...
case:
  ...
  | UNDERSCORE MINUSGREATER valu                  
      { (AtIndex.atc, 
	 mkvalu(T_curry (cons (AtIndex.it, (mkwild(), mktyp(F_x))) nil, $3))) }
;
embedding:
  | valu
      { $1 } 
...
pattern:
  | value_label COLON typ                       { ($1, $3) }
  | LPAREN value_label COLON typ RPAREN         { ($2, $4) }
...
list1_elem_list:
  | valu                                        { [$1] }
  | list1_elem_list SEMI valu                   { $3 :: $1 } /* reversed */
;
valu:
    nonvariant_value 
      { $1 }
  | variant_value 
      { mkvalu(T_comp (mkvalu(T_record nil), $1)) }
  | valu DOT variant_value 
      { mkvalu(T_comp ($1, $3)) }
  | valu DOT nonvariant_value  
      { mkvalu(T_comp ($1, $3)) }
  | valu list1_argument
      { mkvalu(T_appl ($2, $1)) }
  | valu TILDE
...
nonvariant_value:
  ...
  | LPAREN valu error
      { unclosed "(" 1 ")" 3 }
  | LBRACKET list1_elem_list RBRACKET
      { mk_elem_list($2) }
  | LBRACKET list1_elem_list error
      { unclosed "[" 1 "]" 3 }
;
